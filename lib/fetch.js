...
function parse(html) {
  ...
  function pickColor(_i, item) {
    ...
    var hex = $item.find('.hex').text().toLowerCase();
    return { shade: shade, hex: hex };
  }

  function pickGroupColors(_i, group) {
    var $group = $(group);
    var colorName = $group.find('.color-tag .group').text();
    var name = colorName.toLowerCase().split(' ').join('-');
    var $items = $group.find('.color-tag .details');
  ...
  function filterEmpty(_, el) {
    return $(el).text().trim() !== '';
  }

  var $groups = $('#color-color-palette .module').filter(filterEmpty);
